...
def add_edges(beta, gamma, topology, n_paths, demand, fiberhut_capacity, gbps_cost, fiberhut_cost, possible_edges):
    ...
    for r in range(1,len(possible_add)):
        ...
        #pbar.set_description(f'Combinations of {r} edges - cheapest: {cheapest}')
        if cheapest != 0:
        ...
        new_tops = [es for es,_  in new_tops if r*fiberhut_cost < cheapest]
...
for run in range(runs):
    ...
    #possible_edges = {(i,j): weibull() for i in topology.nodes for j in topology.nodes if i!=j and (i,j) not in topology.edges} # TODO: Select some of these
    ...
    # TODO LOG:
    # run demand_scale cost num_added added_Edges
